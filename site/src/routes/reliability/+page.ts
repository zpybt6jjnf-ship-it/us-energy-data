...
export async function load({ fetch }) {
	...
	const byStateTrend = results[2].status === 'fulfilled' ? results[2].value : [];
	const pricesByState = results[3].status === 'fulfilled' ? results[3].value : [];
	const topo = results[4].status === 'fulfilled' ? results[4].value : null;
	...
	for (let i = 0; i < results.length; i++) {
		if (results[i].status === 'rejected') {
			console.warn(`Failed to fetch reliability data [${i}]:`, (results[i] as PromiseRejectedResult).reason);
		}
	}
	...
	return { national, byState, byStateTrend, pricesByState, topology, marketStructure, lastUpdated };
}
